...
public class RmsFile implements SimpleFile 
{
    ...
    public void open(String name, boolean readOnly, boolean noFlush, int storageSizeLimit)
        {
        try { 
            this.name = name;
            readWrite = !readOnly;
            ...
            if (maxStoreId > 0) { 
                ...
                maxPagesInStore = storageSizeLimit/PAGE_SIZE;
            }                
        } catch (Exception x) { 
    ...
    public int read(long pos, byte[] b)
    {
        ...
        while (len > 0) { 
            try { 
                ...
                if (off == 0 && pageOffs == 0 && len == PAGE_SIZE) { 
                    rc = store[sid].getRecord(currPage+1, b, 0);
                    nGets += 1;
                    Assert.that(rc == PAGE_SIZE);
                    return rc;
                }
                ...
                if (!singleStorage && store.length > 1) { 
                    ...
                    throw new StorageError(StorageError.FILE_ACCESS_ERROR, "RmsFile.read(pos=" + pos + ", currPage=" + (currPos >>> PAGE_SIZE_LOG2) + ", nPages=" + nPages + "maxPagesInStore=" + maxPagesInStore + ", nStores=" + store.length + "):" + x);
                }
            } 
        }
        return rc - len;
    ...
    public void write(long pos, byte[] b)
    {
        ...
        while (len > 0) { 
            try { 
                ...
                int n = PAGE_SIZE - pageOffs < len ? PAGE_SIZE - pageOffs : len;
                if (currPage >= nPages) { 
                    if (currPage > nPages || n != PAGE_SIZE) { 
                ...
                if (currPage >= nPages) {
                    try { 
                        ...
                        while (currPage > nPages) { 
                            id = store[sid].addRecord(pageData, 0, PAGE_SIZE);                            
                            nAdds += 1;
                            nPages += 1;
                            Assert.that(id == nPages);
                        }
                        if (n != PAGE_SIZE) { 
                            System.arraycopy(b, off, pageData, pageOffs, n);
                            id = store[sid].addRecord(pageData, 0, PAGE_SIZE);
                        } else { 
                            id = store[sid].addRecord(b, off, PAGE_SIZE);
                        }
                        nAdds += 1;
                        nPages += 1;
                        Assert.that(id == nPages);
                        ...
                        int i = store.length; 
                        int nRecords = 0;
                        while (--i >= 0 && (store[i] == null || (nRecords = store[i].getNumRecords()) == 0));
                        if (i != 0 || nRecords > nPages) { 
                            throw new StorageError(StorageError.FILE_ACCESS_ERROR, "Space is exhausted");
                        }
                        System.err.println("RecordStoreFullException: " + nPages + " pages");
                        maxPagesInStore = nPages;
                        continue;
                    }
                } else { 
                    if (pageData != b) { 
                        ...
                        store[sid].setRecord(currPage+1, b, off, PAGE_SIZE);
                    }
                    nSets += 1;
                }
                len -= n;
                ...
                System.err.println("RecordStoreException: currPage=" + (currPos  >>> PAGE_SIZE_LOG2) + ", nPages=" + nPages);
                x.printStackTrace();
                if (!singleStorage && store.length > 1) { 
                    singleStorage = true;
                } else { 
                    throw new StorageError(StorageError.FILE_ACCESS_ERROR, "RmsFile.write(pos=" + pos + ", currPage=" + (currPos >>> PAGE_SIZE_LOG2) + ", nPages=" + nPages + "maxPagesInStore=" + maxPagesInStore + "):" + x);
                }
            } 
        }
        if (currPos > size) { 
