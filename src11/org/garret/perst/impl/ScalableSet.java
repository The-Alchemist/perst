...
public class ScalableSet extends PersistentResource implements IPersistentSet { 
    ...
    public Object[] toArray() { 
        // Blackberry verificator failure
    ...
    public Object[] toArray(Object a[]) { 
        ...
        if (link != null) {
            return link.toArray((IPersistent[]) a);
        }  else {
    ...
    private int binarySearch(IPersistent obj) { 
        int l = 0, r = link.size();
        int oid = obj.getOid();
        while (l < r) { 
            int m = (l + r) >> 1;
            if (link.getRaw(m).getOid() > oid) { 
                l = m + 1;
            } else { 
                r = m;
            }
    ...
    public Iterator join(Iterator with) { 
        return with == null ? iterator() : new JoinSetIterator(getStorage(), iterator(), with);
    }    
}
