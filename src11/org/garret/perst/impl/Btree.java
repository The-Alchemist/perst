...
public class Btree extends PersistentResource implements Index { 
    ...
    {            
        ...
        if (result == op_underflow) { 
            ...
            if (BtreePage.getnItems(pg) == 0) {                         
                int newRoot = 0;
                if (height != 1) { 
                    newRoot = (type == Types.String || (type == Types.ArrayOfByte && BtreePage.getSize(pg) >= 0)) 
    ...
    static final String MAX_CHAR = new Character(Character.MAX_VALUE).toString();

    public IPersistent[] getPrefix(String prefix) { 
        return get(new Key(prefix.toCharArray(), true), 
                   new Key((prefix + MAX_CHAR).toCharArray(), false));
    }
    ...
    Object unpackByteArrayKey(Page pg, int pos) {
        int len = BtreePage.getSize(pg);
        int offs;
        if (len < 0) { 
            len = -len;
            offs = BtreePage.firstKeyOffs + pos*len;
        } else {
            len = BtreePage.getKeyStrSize(pg, pos);
    ...
    class BtreeSelectionIterator extends Iterator { 
        ...
        void reset() { 
            ...
            } else if (type == Types.ArrayOfByte && fixedSize == 0) { 
                if (order == ASCENT_ORDER) { 
                    if (from == null) { 
                        while (--h >= 0) { 
                            posStack[sp] = 0;
        ...
        private int getReference(Page pg, int pos) { 
            return (type == Types.String || (type == Types.ArrayOfByte && fixedSize == 0))
            ...
            if (sp != 0) { 
                if (nextKey == null) { 
                    ...
                    while (true) { 
                        int pos = posStack[sp-1];   
                        Page pg = db.getPage(pageStack[sp-1]);
                        int oid = type == Types.String || (type == Types.ArrayOfByte && fixedSize == 0)
                            ? BtreePage.getKeyStrOid(pg, pos)
                            : BtreePage.getReference(pg, BtreePage.maxItems-1-pos);
                        if (oid != next) { 
                            gotoNextItem(pg, pos);
        ...
        BtreeKey getCurrentKey(Page pg, int pos) { 
            ...
            case Types.ArrayOfByte:
                if (fixedSize == 0) { 
                    ...
                    byte[] arr = new byte[fixedSize];
                    key = new BtreeKey(new Key(arr, fixedSize), 
                                       BtreePage.getReference(pg, BtreePage.maxItems-1-pos));
                    System.arraycopy(pg.data, BtreePage.firstKeyOffs + pos*fixedSize, arr, 0, fixedSize);
                }
                break;
            default:
    ...
    class BtreeSelectionEntryIterator extends BtreeSelectionIterator { 
        ...
        protected Object getCurrent(Page pg, int pos) { 
            ...
            switch (type) { 
              ...
              case Types.ArrayOfByte:
                return new BtreeEntry(db, unpackByteArrayKey(pg, pos), fixedSize != 0 
                                      ? BtreePage.getReference(pg, BtreePage.maxItems-1-pos) 
                                      : BtreePage.getKeyStrOid(pg, pos));
              default:
    ...
    public Iterator prefixIterator(String prefix, int order) {
        return iterator(new Key(prefix.toCharArray()), 
                        new Key((prefix + MAX_CHAR).toCharArray(), false), order);
    }
