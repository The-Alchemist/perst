...
class BtreePage { 
    ...
    static int getItemSize(Page pg, int type) { 
        return type == Types.ArrayOfByte ? -getSize(pg) : ClassDescriptor.sizeof[type];
    }
        ...
        switch (key.type) {
          ...
          case Types.Double:
           ...
           case Types.ArrayOfByte:
            size = -getSize(pg);
            return Arrays.compare(pg.data, BtreePage.firstKeyOffs + i*size, (byte[])key.oval, 0, size);
        }
                        ...
                        Btree tree, int height, ArrayList result)
    {
        ...
        try { 
            ...
            } else if (tree.type == Types.ArrayOfByte && getSize(pg) >= 0) { 
                if (firstKey != null) { 
                    while (l < r)  {
                        int i = (l+r) >> 1;
                      ...
                      boolean unique, boolean overwrite)
    {
        ...
        try { 
            if (tree.type == Types.String) {         
                ...
                pg = null;
                pg = db.putPage(pageId);
                int itemSize = getItemSize(pg, tree.type);
    ...
    static int handlePageUnderflow(StorageImpl db, Page pg, int r, int type, BtreeKey rem, int height)
    {
        ...
        if (type == Types.String) { 
        ...
        } else if (type == Types.ArrayOfByte && getSize(pg) >= 0) { 
            Page a = db.putPage(getKeyStrOid(pg, r));
            int an = getnItems(a);
            if (r < nItems) { // exists greater page
                Page b = db.getPage(getKeyStrOid(pg, r+1));
            ...
            int an = getnItems(a);
            int itemSize = getItemSize(pg, type);
            if (r < nItems) { // exists greater page
    ...
    static void purge(StorageImpl db, int pageId, int type, int height)
    {
        if (--height != 0) { 
            ...
            if (type == Types.String || (type == Types.ArrayOfByte && getSize(pg) >= 0)) { // page of strings
                while (--n >= 0) { 
