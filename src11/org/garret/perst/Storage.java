...
public interface Storage { 
     ...
     * @param keyType map key type (org.garret.perst.Types enumeration)
     * @param smallMapSize size of built-in hash map, when number of elements exeeds half of this size, B-Tree implementation
     * is used instead of embedded hash map.
     * @return scalable map implementation
     */
    public IPersistentMap createMap(int keyType, int smallMapSize);
     ...
     * <TD>Number of objects pinned in LRU object cache or size of STRONG object cache after clearing.
     * </TD></TR>
     * <TR><TD><code>perst.object.cache.init.size</code></TD><TD>Integer</TD><TD>1319</TD>
     * <TD>Initial size of object cache
     * </TD></TR>
     * <TR><TD><code>perst.object.cache.kind</code></TD><TD>String</TD><TD>"lru"</TD>
     * <TD>Kind of object cache. Currently "strong" and "weak" cache are supported by Perst.Lite. 
     * <B>Strong</B> cache uses strong (normal) references to refer persistent objects. 
     * Thus none of loaded persistent objects can be deallocated by GC. 
     * It is possible to explicitely clear object cache using 
     * <code>Storage.clearObjectCache()</code> method. Also cache is cleared at the end of transaction.<br>
     * <B>Weak</B> cache uses weak references but pins some limited number of objects in memory.
     * Since there is no finalization mechanism in CLDC, weak object cache holds all modified objects in memory.
     * Also most frequently used not-modified objects can be pinned in memory. Weak object cache uses
     * LRU discipline for pinning objects. Number of pinned objects is determined 
     * by <code>perst.object.cache.pin.count</code> parameter (it can be 0).
     * </TD></TR>
     ...
     * </TD></TR>
     * <TR><TD><code>perst.record.store.limit</code></TD><TD>Integer</TD><TD>0</TD>
     * <TD>Specify maximal size of RMS storage or JSR75 file. Perst will automatically create new stores on demand
     * when more space is needed. Actually RMS file implementation contains mechanism which automatically detects
     * record store overflow, but at some platforms (like S60 5th edition) RecordStoreFullException is not thrown
     * but record store is corrupted. Value 0 of this parameter means that record store size is unlimited.
     * </TD></TR>
     * <TR><TD><code>perst.reload.objects.on.rollback</code></TD><TD>Boolean</TD><TD>false</TD>
     * <TD>By default Perst doesn't reload modified objects during transaction rollback.
     * Programmer in this case should not use references to the persistent objects stored in program variables.
     * Instead of it, application should fetch object tree from the beginning, starting from 
     * obtaining root object using Storage.getRoot method.
     * Setting this property to true instructs Perst to reload all objects modified by the rollbacked transaction.
     * It certainly takes some additional amount of time, but in this case it is not necessary 
     * to forgot references stored in variables, unless them point to the objects created by this transactions.
     * Unfortunately there is no way to prohibit access to such objects or somehow invalidate references
     * to them. So this option should be used with care. 
     * </TD></TR>
     * </TABLE>
