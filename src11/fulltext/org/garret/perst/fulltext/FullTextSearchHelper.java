...
public class FullTextSearchHelper extends Persistent
{
    ...
    public boolean isWordChar(char ch) { 
        ...
        //return (ch >= 'a' && ch <= 'z') || (ch >= 'A' && ch <= 'Z') || (ch >= '0' && ch <= '9') && ch > 0xFF;
    }
    ...
    public void writeObject(IOutputStream out) {
        out.writeFloat(nearnessWeight);
        out.writeInt(maxWordLength);
        out.writeInt(wordSwapPenalty);
        out.writeString(AND);
        out.writeString(OR);
        out.writeString(NOT);
        out.writeArrayOfString(stopWords);
    }

    public void readObject(IInputStream in) {
        nearnessWeight = in.readFloat();
        maxWordLength = in.readInt();
        wordSwapPenalty = in.readInt();
        AND = in.readString();
        OR  = in.readString();
        NOT = in.readString();
        stopWords = in.readArrayOfString();
    }
