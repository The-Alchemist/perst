...
public final class ClassDescriptor extends Persistent { 
    ...
    void buildFieldList(StorageImpl storage, Class cls, ArrayList list) { 
        ...
        if (storage.getDatabaseFormatVersion() >= 2) { 
            Arrays.sort(flds, new Comparator() { public int compare(Object f1, Object f2) { return ((Field)f1).getName().compareTo(((Field)f2).getName()); } });
        ...
        for (int i = 0; i < flds.length; i++) { 
            Field f = flds[i];
            // Ignore syntetic fields (which are generated by some JVMs even for not-nested classes
            if (!f.getName().startsWith("class$") && 
                (f.getModifiers() & (Modifier.TRANSIENT|Modifier.STATIC)) == 0) 
            {
                try { 
