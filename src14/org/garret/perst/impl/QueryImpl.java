...
class FilterIterator implements Iterator { 
    ...
    public boolean hasNext() { 
        ...
        while (iterator.hasNext()) { 
            ...
            if (query.cls.isInstance(currObj)) { 
                if (condition == null) { 
                    return true;
                }
                try { 
...
class Node { 
    ...
    final static int tpTable      = 19;
    final static int tpUnknown    = 20;
    final static int tpAny        = 21;
...
class BinOpNode extends Node { 
    ...
    boolean evaluateBool(FilterIterator t) {
        switch (tag) { 
          ...
          case opScanTable:
          {
            Iterator iterator = ((ITable)right.evaluateObj(t)).iterator();
            Object mbr = left.evaluateObj(t);
            while (iterator.hasNext()) { 
                if (iterator.next() == mbr) { 
                    return true;
                }
            }            
            return false;
...
class ContainsNode extends Node implements Comparator { 
    ...
    boolean evaluateBool(FilterIterator t) { 
        ...
        if (havingExpr != null) { 
            ...
            t.containsArray = null; // prevent reuse of the same array by nested CONTAINS in with expression
        }
        Object saveContainsElem = t.containsElem;

        if (collection instanceof Collection || collection instanceof ITable) { 
            Object elem;
            Iterator iterator = collection instanceof Collection 
                ? ((Collection)collection).iterator()
                : ((ITable)collection).iterator();
            if (withExpr != null) { 
                while (iterator.hasNext()) { 
                    elem = iterator.next();
                    if (elem != null) { 
                ...
                for (len = 0; iterator.hasNext(); len++) { 
                    elem = iterator.next();
...
class OrderNode {
    ...
    final int compare(Object a, Object b) { 
        ...
        try {
            if (method != null) {
                diff = ((Comparable)method.invoke(a, null)).compareTo(method.invoke(b, null));
            } else { 
                switch (type) { 
...
public class QueryImpl implements Query {
    ...
    {       
        Iterator result = applyIndex(tree);
        if (result == null) { 
    ...
    private final GenericIndex getIndex(String key) { 
            ...
            || cop == tknLike || cop == tknNot || cop == tknIs || cop == tknIn)
        {
            ...
            if (cop == tknIn) { 
                ...
                if (right.type != Node.tpList && (left.type == Node.tpAny || right.type == Node.tpAny)) { 
                    ...
                    switch (right.type) {
                      case Node.tpCollection:
                        ...
                        break;
                      case Node.tpTable:
                        left = new BinOpNode(Node.tpBool, Node.opScanTable,
                                             left, right);
                        break;
                ...
                if (left.type == Node.tpAny || right.type == Node.tpAny) {
                    ...
                    left = new BinOpNode(Node.tpBool, Node.opStrEq+cop-tknEq, left, right);
                } else if (left.type == Node.tpDate) {
                    if (right.type == Node.tpStr) {
                        right = str2date(right);
                    } else if (right.type != Node.tpDate) {
                    ...
                    System.err.println("left.type=" + left.type + ", right.type=" + right.type);
        ...
        while (true) { 
            ...
            switch (lex) {
              case tknDot:      
                ...
                if (expr.type != Node.tpObj && expr.type != Node.tpAny && expr.type != Node.tpCollection && expr.type != Node.tpTable) { 
                ...
                if (contains != null && contains.containsExpr.equals(expr)) { 
                    ...
                    if (expr.type == Node.tpCollection || expr.type == Node.tpTable) { 
                        throw new CompileError("Left operand of '.' should be reference", p);
                    }                        
    ...
    final Node containsElement() {
        ...
        if (arrClass == null || (!arrClass.isArray() && !arrClass.equals(Object.class) 
                                 ...
                                 && !(ITable.class.isAssignableFrom(arrClass)))) 
        { 
    ...
    final Iterator applyIndex(Node condition) 
    {
