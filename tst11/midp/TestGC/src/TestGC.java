...
public class TestGC extends MIDlet implements CommandListener
{ 
    ...
    static final int N_INSERT_RECORDS = 10000;
    static final int GAUGE_UPDATE_INTERVAL = 100;
    static final int PAGE_POOL_SIZE = 2*1024*1024;
    static final int BUSY_MSG_TIMEOUT = 1000;
    static final int RUN_TIMEOUT = 1000;

    static class GCListener extends StorageListener 
    {
        int nDeallocatedObjects;

        public void gcCompleted(int nDeallocatedObjects) {
            this.nDeallocatedObjects = nDeallocatedObjects;
        }
    }
    ...
    protected void startApp() 
    {
        ...
        try { 
            storage.open("testgc.dbs", PAGE_POOL_SIZE);
    ...
    abstract class RunTestThread extends Thread { 
        ...
        protected boolean increment(int i) { 
            if (stopped) { 
                new ResultForm(TestGC.this, form.getTitle(), i, System.currentTimeMillis() - startTime);
                return false;
            } else { 
        ...
        public void run() { 
            ...
            if (runTest()) { 
                if (form.progress == null) {
                    ...
                    new ResultForm(TestGC.this, form.getTitle(), form.progress.getMaxValue(), System.currentTimeMillis() - startTime);
                }
            }
            synchronized (mutex) { 
                busy = false;
    ...
    class InsertThread extends RunTestThread { 
        ...
        public boolean runTest() { 
            ...
            storage.gc();
            Assert.that(listener.nDeallocatedObjects == 0);
            storage.commit();
            return true;
        }
    }
    ...
    class RemoveThread extends RunTestThread { 
        ...
        public boolean runTest() { 
            ...
            storage.setListener(listener);
            storage.gc();
            Assert.that(listener.nDeallocatedObjects == n);
            storage.commit();
            return true;
        }
    }
