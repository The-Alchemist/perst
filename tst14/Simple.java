...
class MyRootClass extends Persistent 
{
    ...
    public MyRootClass(Storage db) { 
        ...
        intKeyIndex = db.createFieldIndex(
                                          ...
                                          true); // unique index
        strKeyIndex = db.createFieldIndex(MyPersistentClass.class, // class for which index is defined
                                          "strKey",  // name of indexed field
                                          false); // index allows duplicates (is not unique)
        foreignIndex = db.createIndex(int.class, // key type
                                      false); // index allows duplicates (is not unique)
    }
...
public class Simple extends Persistent 
{
    ...
    static public void main(String[] args) {    
        ...
        // "Persistence by reachability" apporachm which means that persistent capable class with
        ...
        obj = (MyPersistentClass)root.intKeyIndex.get(new Key(1));
        ...
        Key key = new Key("A.B"); 
        IPersistent[] result = root.strKeyIndex.get(key, key);
        for (int i = 0; i < result.length; i++) { 
            System.out.println("Exact match search by strKey: " + result[i]);
        }
        
        // Get iterator through records belonging to specified key range in ascent order
        Iterator iterator = root.foreignIndex.iterator(
                                                       new Key(100, true), // inclusive low boundary
                                                       new Key(10000, false), // exclusive high boundary
                                                       Index.ASCENT_ORDER); // ascent order 
        while (iterator.hasNext()) { 
            obj = (MyPersistentClass)iterator.next();
            System.out.println("Range search by foreign key: " + obj);
        }

        // Locate all objects which strKey starts with prefix "A."
        iterator = root.strKeyIndex.prefixIterator("A.");
        while (iterator.hasNext()) { 
            obj = (MyPersistentClass)iterator.next();
            System.out.println("Search by prefix: " + obj);
        }
        
        // Locate all objects which strKey is prefix of "A.B.C"
        result = root.strKeyIndex.prefixSearch("A.B.C");
        for (int i = 0; i < result.length; i++) { 
            System.out.println("Locate prefix: " + result[i]);
        }
